...
| **E-Mail-Adresse geändert** | `updateParticipant` | Neue Adresse (Änderung + Reset-Link); alte Adresse (Security) | `buildEmailChangedNewAddressMail` (mit `passwordResetLink`), `buildEmailChangedOldAddressMail` | Bei `active`: Global Sign-Out, altes Passwort ungültig, **eine** Kombi-Mail an neu + Hinweis an alt (#350) |
