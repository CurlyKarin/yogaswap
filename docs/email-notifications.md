...
Technik: **AWS SES** — `SendEmail` (HTML) und `SendRawEmail` (HTML + `.ics`-Anhang). Absender `SES_SOURCE_EMAIL` (Rollout: `noreply@yogaswap.de`). Cognito-Versand ist überall unterdrückt (`MessageAction: SUPPRESS`); Auth-Mails laufen über eigene Token-Links. Sandbox → Production und Domain-DNS: [`docs/ses-production.md`](./ses-production.md) (#80).
