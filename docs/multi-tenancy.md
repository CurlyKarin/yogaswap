...
- **User**  
  ...
  - **E-Mail:** bewusst nicht eindeutig — mehrere Cognito-Accounts / Profile mit derselben Adresse sind erlaubt (Tests). Auto-Verknüpfung per E-Mail ist **nicht** Default; explizites Multi-Studio-Linking folgt später in der UI.
...
**Doppelanlage / kein Profil-Merge (#325 geschlossen):** Ein No-Login-Profil oder ein gerade hinzuzufügendes Profil wird **nicht** mit einem bereits bestehenden Mitglied im selben Studio technisch zusammengeführt — das ist Absicht, kein fehlendes Feature. Bei zwei Personen-Zeilen im Studio (Tippfehler, Doppelanlage) löst die Studioleitung das **operativ**: redundante Person aus dem Kurs abmelden, die eigentliche Person dem Kurs zuordnen. Es gibt keinen `mergeParticipants`-Flow und kein Umbiegen von Kurs-/Enrollment-/Swap-Referenzen zwischen zwei `userId`s im selben Tenant. **Weiter erlaubt:** Verknüpfen mit einem Cognito-Konto, das hier noch nicht Mitglied ist (z. B. anderes Studio oder Profil ohne Membership) — siehe #342.

**Studio-Exit (#329 / #271):** `DELETE /participants/{userId}` entfernt die Membership erst, wenn **keine Zukunft** mehr geplant ist: offene/noch gültige Stamm-Enrollments in **active** oder **draft**-Kursen, zukünftige Swaps, Warteliste auf Zukunftsterminen. **Inactive**-Kurse blockieren nicht (Mitglieder dort nicht editierbar; Soft-Delete räumt mit auf). Veralteter `course.participants[]`-Cache allein blockiert nicht (Aufräumen beim Soft-Delete). Sonst **409** `studio_exit_blocked` mit Liste. Vorschau: dieselbe Route mit `?check=1` (read-only). Kein Auto-Storno im MVP — Admin räumt vorher auf. Historie darf stehen bleiben.
