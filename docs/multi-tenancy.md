...
**Studio-Exit (#329):** `DELETE /participants/{userId}` entfernt die Membership erst, wenn **keine Zukunft** mehr geplant ist: offene/noch gültige Stamm-Enrollments (bzw. Stamm-Cache), zukünftige Swaps, Warteliste auf Zukunftsterminen. Sonst **409** `studio_exit_blocked` mit Liste. Vorschau: dieselbe Route mit `?check=1`. Kein Auto-Storno im MVP — Admin räumt vorher auf. Historie darf stehen bleiben.
