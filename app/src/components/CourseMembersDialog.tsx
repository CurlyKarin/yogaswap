...
import { filterParticipantsBySearch, getStatusPresentation, resolveParticipantRef, type ParticipantStatusPresentation } from "../lib/participants";
import {
...
export default function CourseMembersDialog({
  ...
  const availableParticipants = useMemo(
    () =>
      participants
        .filter((entry) => (entry.role ?? "participant") === "participant")
        .map((entry) => ({
          ...entry,
          participantId: resolveParticipantRef(entry),
        })),
    [participants],
  );
  ...
  const filteredParticipants = useMemo(() => {
    ...
    return searched.filter((entry) => {
      const key = entry.participantId.toLowerCase();
      return !rosterIds.has(key) && !formerIds.has(key);
    });
  }, [availableParticipants, search, isIntervalUi, rosterIds, formerIds]);

  const profileById = useMemo(
    () => new Map(availableParticipants.map((entry) => [entry.participantId.toLowerCase(), entry])),
  ...
  const handleParticipantListKeyDown = (event: KeyboardEvent<HTMLDivElement>) => {
    ...
    if (event.key === " " || event.key === "Enter") {
      ...
      toggleDraftParticipant(active.participantId);
    }
  };
      ...
      <div className="dialog-stack">
              ...
              >
                {filteredParticipants.map((entry, index) => {
                  const checked = selectedParticipants.some(
                    (value) => value.toLowerCase() === entry.participantId.toLowerCase(),
                  );
                  ...
                  return (
                    <div
                      ...
                      id={`participant-option-${entry.participantId.toLowerCase()}`}
                      role="option"
                      ...
                      onClick={() => handleParticipantOptionClick(index, entry.participantId)}
                    >
                      <span
                        ref={(element) => {
              ...
              >
                        ...
                        {!isInactive && rejoinOptions.length > 0 && (
                          <label className="course-members-date">
                            ...
                            <select
                              ...
                              onChange={(event) => {
                                const next = event.target.value;
                                if (next) addIntervalMember(row.participantId, next);
                              }}
                            >
                ...
                {!isInactive &&
                  filteredParticipants.map((entry) => {
                    const status = getStatusPresentation(entry.status);
                    return (
                      <div key={entry.participantId} className="course-members-row">
                        <MemberIdentity userId={entry.userId} email={entry.email} status={status} />
                        <label className="course-members-date">
                          ab
                          <select
                            value=""
                            disabled={saving}
                            aria-label={`${entry.participantId} gültig ab`}
                            onChange={(event) => {
                              const next = event.target.value;
                              if (next) addIntervalMember(entry.participantId, next);
                            }}
                          >
                            <option value="">wählen</option>
        ...
        {participantsLoaded && isDraft && (
          <StaleNote
            ...
            availableIds={new Set(availableParticipants.map((entry) => entry.participantId.toLowerCase()))}
          />
        )}
